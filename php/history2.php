<?php
    ...
    if($_SESSION["CorP"]=="C"){
        if($_REQUEST['from']==''){
            ...
            // str_replace( "T" , " " , $to , 1 );
            $sql = "SELECT name as 地點名稱,time as 時間,is_epidemic as 是否為疫區 FROM footprint_data f,place p WHERE f.place_id=p.place_id AND f.time BETWEEN '".$from."' AND '".$to."' AND f.customer_id=".$_SESSION["id"]." ORDER BY time";
        }
    }
    else{
        if($_REQUEST['from']==''){
